<script setup lang="ts">
import type { NavigationMenuItem } from "@nuxt/ui/components/NavigationMenu.vue"
import "./header.scss"

const route = useRoute();
const items = computed<NavigationMenuItem[]>(() => [
  {
    label: "Home",
    to: `/`,
    active: route.path.endsWith("/"),
  },
  {
    label: "Merchandise",
    to: `/merch`,
    active: route.path.endsWith("/merch"),
  },
  {
    label: "Schedule",
    to: `/schedules`,
    active: route.path.startsWith("/schedules"),
  },
  {
    label: "Standing",
    to: `/standings`,
    active: route.path.startsWith("/standings"),
  },
  {
    label: "Venue And Vendors",
    to: `/venueAndVendors`,
    active: route.path.startsWith("/venueAndVendors"),
  },
  {
    label: "About Us",
    to: `/aboutus`,
    active: route.path.startsWith("/aboutus"),
  },
]);
</script>

<template>
  <UHeader
    class="header"
    toggle-side="left"
    :ui="{
    toggle: 'flex sm:hidden',
    }"
  >
    <template #left>
      <img
        src="~/assets/images/aachen-open-logo.svg"
        alt="logo"
        class="header-logo size-15"
      />
      <h1 class="header-text text-2xl font-bold">
        Aachen Open
      </h1>
    </template>

    <template #right>
      <div class="hidden sm:flex">
        <UNavigationMenu
          :items="items"
          color="neutral"
          highlight
          class="header-menu"
          :ui="{
            link: 'header-menu'
          }"
        />
      </div>
    </template>

    <template #body>
      <UNavigationMenu
        :items="items"
        color="neutral"
        highlight
        class="header-menu"
        orientation="vertical"
        :ui="{
          link: 'header-menu text-xl'
        }"
      />
    </template>
  </UHeader>
</template>

<style scoped>
</style>
